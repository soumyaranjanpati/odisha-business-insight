...
CREATE INDEX idx_article_views_article_viewed ON public.article_views(article_id, viewed_at);

COMMENT ON TABLE public.article_views IS 'One row per article page view; used for admin analytics.';

-- RLS: anyone can insert (API records views from anonymous visitors); only admin can read
